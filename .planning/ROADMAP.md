...
**Success Criteria** (what must be TRUE):
  1. The Home page shows a rotating hero carousel (3–5 landscape placeholders, 7s visible + 1s crossfade per slide), an overlay with group name / tagline / affiliation that passes WCAG-AA contrast on every slide, an intro block, 3 highlight cards, and a partner logo strip; the carousel exposes a visible pause button, pauses when the tab is backgrounded (`document.hidden`), and does not auto-advance when `prefers-reduced-motion` is set. Hover and keyboard focus deliberately do NOT pause — the pause button is the single explicit affordance (HOME-01..07).
